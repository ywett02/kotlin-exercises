...
class Registry {
    private val dependencies = mutableMapOf<KType, () -> Any?>()
    private val singletons = mutableMapOf<KType, Any?>()

    inline fun <reified T> singleton(noinline init: Registry.() -> T) {
        singleton(typeOf<T>(), init)
    }

    fun singleton(kType: KType, init: Registry.() -> Any?) {
        singletons[kType] = init.invoke(this)
    }

    inline fun <reified T : Any> register(noinline init: Registry.() -> T) {
        register(typeOf<T>(), init)
    }

    fun register(clazz: KType, init: Registry.() -> Any?) {
        dependencies[clazz] = { init(this) }
    }
    ...
    fun get(type: KType): Any? =
        when {
            singletons.contains(type) -> singletons[type]
            dependencies.contains(type) -> dependencies[type]?.invoke()
            else -> throw IllegalArgumentException("There is no instance of $type")
        }

    inline fun <reified T : Any> exists() =
        exists(typeOf<T>())

    fun exists(type: KType): Boolean {
        return singletons.contains(type) || dependencies.contains(type)
    }
}
